    ...
    def HorizVert(grid_horiz_vert):
        ...
        for i in range(len(grid_horiz_vert)):
            for j in range(0, len(grid_horiz_vert[0])):
                try:
                    sumx = int(grid_horiz_vert[i][j]) * int(grid_horiz_vert[i][j + 1]) * int(
                        grid_horiz_vert[i][j + 2]) * int(grid_horiz_vert[i][j + 3])
                    if sumx > biggestx:
                        biggestx = sumx
                except KeyError:
                    pass
        for i in range(len(grid_horiz_vert)):
            for j in range(0, len(grid_horiz_vert[0])):
                try:
                    sumy = int(grid_horiz_vert[j][i]) * int(grid_horiz_vert[j][i + 1]) * int(
                        grid_horiz_vert[j][i + 2]) * int(grid_horiz_vert[i][i + 3])
                    if sumy > biggesty:
                        biggesty = sumy
                except KeyError:
                    pass
        ...
        for j in range(len(grid_diagonal)):
            for i in range(len(grid_diagonal)):
                try:
                    ...
                    b = int(grid_diagonal[i + j + 1][len(grid_diagonal) - i - 1])
                    c = int(grid_diagonal[i + j + 2][len(grid_diagonal) - i - 2])
                    d = int(grid_diagonal[i + j + 3][len(grid_diagonal) - i - 3])
                    product = a * b * c * d
                    if product > b4:
                        b4 = product
                ...
                try:
                    ...
                    b = int(grid_diagonal[i - 1][len(grid_diagonal) - i - j - 1])
                    c = int(grid_diagonal[i - 2][len(grid_diagonal) - i - j - 2])
                    d = int(grid_diagonal[i - 3][len(grid_diagonal) - i - j - 3])
                    product = a * b * c * d
                    if product > b2:
                        b2 = product
...
def euler_32():
    ...
    for i in range(len(all_4_digits)):
        for key in list(all_prime_factors[i].keys()):
            all_combined = str(all_4_digits[i]) + str(key) + str(all_prime_factors[i][key])
            if is_pandigital(all_combined):
                print(f"{key} * {all_prime_factors[i][key]} = {all_4_digits[i]}")
...
def euler_33():
    ...
    for fraction in fractions:
        ...
        if (numerator[0] != numerator[1]) and (denominator[0] != denominator[1]) and set(numerator) & set(denominator):
            ...
            for shared in shared_digits:
                ...
                if int(new_num) < int(new_den) and "0" not in [new_num, new_den] and not zeros_in_both_original:
                    if int(numerator) / int(denominator) == int(new_num) / int(new_den):
...
def euler_37():
    ...
    while number_found < 11:
        if is_prime_as_truncate_l_to_r(str(n)) and is_prime_as_truncate_r_to_l(str(n)) and is_prime(n):
            print(n)
            found.add(n)
            number_found += 1
        n += 1
